...
class Todo:
    def __init__(self,task,category,
        ...
        if date_added is not None else datetime.datetime.now().isoformat()
        self.date_completed = date_completed\
        if date_completed is not None else None
        self.status = status\
        if status is not None else 1 ##0=open/not-completed #1=completed
